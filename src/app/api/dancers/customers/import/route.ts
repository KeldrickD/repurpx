...
const ImportRowSchema = z.object({
  ...
  tier: z.nativeEnum(DancerCustomerTier).optional(),
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const prepared = data.rows.map((row) => ({
      ...
      tier: row.tier ?? DancerCustomerTier.REGULAR,
    ...
    const result = await prisma.dancerCustomer.createMany({
      data: prepared,
    });
    ...
    console.error("[dancers.customers.import]", error);
    const message =
      error instanceof Error ? error.message : "Failed to import dancer customers";
